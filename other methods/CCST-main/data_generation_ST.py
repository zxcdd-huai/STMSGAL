...
def draw_map(generated_data_fold):
    ...
    n_types = len(np.unique(np.array(cell_types)))
    ...
    plt.savefig(generated_data_fold+'/spacial.png', dpi=200, bbox_inches='tight')
    plt.clf()
...
def main(args):
    ...
    for proj_idx in range(len(proj_list)):
        ...
        data_fold = f'/data/pxh/SEDR/data/{args.data_name}/'
        generated_data_fold = args.generated_data_path + args.data_name + '/'

        adata_h5 = st.Read10X(path=data_fold, count_file='filtered_feature_bc_matrix.h5')
        #adata_h5 = sc.read_visium(os.path.join(data_fold))
        print(adata_h5)
        #count = adata_h5.X
        ...
        cell_types = df_meta['layer_guess']
...
if __name__ == "__main__":
    ...
    parser.add_argument( '--data_name', type=str, default='Ductal_Carcinoma_in_situ', help='The name of dataset')
